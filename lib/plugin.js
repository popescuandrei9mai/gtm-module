...
function gtmServer(ctx, initialized) {
  ...
  ctx.beforeNuxtRender(() => {
    ...
    const gtmScript = typeof ctx.app.head === 'function' ? 
    ctx.app.head().script.find(s => s.hid == '<%= options.scriptId %>'): 
    ctx.app.head.script.find(s => s.hid == '<%= options.scriptId %>')
...
<% if (options.noscript) { %>
    const gtmIframe = typeof ctx.app.head === 'function' ? 
    ctx.app.head().noscript.find(s => s.hid == '<%= options.noscriptId %>'):
    ctx.app.head.noscript.find(s => s.hid == '<%= options.noscriptId %>')
